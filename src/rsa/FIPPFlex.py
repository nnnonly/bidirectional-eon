...
class FIPPFlex(RSA):
    ...
    def find_working_path(self, flow: Flow, demand_in_slots: int):
        ...
        for i in range(0, self.pt.get_num_slots() - demand_in_slots ):
            ...
            try:
                ...
                if weight_best_path > len(path) - 1:
                    best_path = path
                    weight_best_path = len(path) - 1
                    best_slot_index = i
            except nx.NetworkXNoPath:
                continue
        return best_path, best_slot_index

    def FIPPFlexAl(self, flow: Flow, demand_in_slots: int):
        working_path, slot_index = self.find_working_path(flow, demand_in_slots)
        if working_path is None:
            ...
            for pcycle in self.vt.get_p_cycles():
                if pcycle.p_cycle_contains_flow(flow.get_source(), flow.get_destination()) and pcycle.get_reserved_slots() >= demand_in_slots:
                    if pcycle.can_add_links_disjoint(wp_links):
                        return pcycle, wp_links, wp_slot_list
            ...
            if path_1_pcycle is not None and path_2_pcycle is not None:
                ...
                return new_p_cycle, wp_links, wp_slot_list
        return None, None, None
    ...
    def flow_arrival(self, flow: Flow) -> None:
        ...
        p_cycle, wp_links, wp_slot_list = self.FIPPFlexAl(flow, demand_in_slots)
        if p_cycle is not None:
            establish, lp_id = self.establish_connection(links=wp_links, slot_list=wp_slot_list, flow=flow, pcycle=p_cycle)
            ...
            for edge in wp_links:
                self.pt.release_slots(self.pt.get_src_link(edge), self.pt.get_dst_link(edge), wp_slot_list)
        ...
        for j in range(0, self.pt.get_num_slots() - demand_in_slots):
            ...
            for u, v, edge_data in new_graph.edges(data=True):
                ...
                if all(spectrum[0][j:j + demand_in_slots]):
                    search_graph.add_edge(u, v, weight_search=1)
            try:
                path1, path2 = self.get_two_shortest_disjoint_paths(flow, search_graph)
                if path1 and path2:
                    if weight_best_path > len(path1) + len(path2) - 2:
                        best_path_1 = path1
                        best_path_2 = path2
                        weight_best_path = len(path1) + len(path2) - 2
                        best_slot_index = j
            except nx.NetworkXNoPath:
                continue
    ...
    def get_two_shortest_disjoint_paths(self, flow: Flow, search_graph: nx.Graph):
        try:
            ...
            path1 = nx.dijkstra_path(search_graph, source=flow.get_source(), target=flow.get_destination(), weight='weight_search')
            ...
            path2 = nx.dijkstra_path(G_copy, source=flow.get_source(), target=flow.get_destination(), weight='weight_search')
